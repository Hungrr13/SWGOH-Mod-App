...
### GAC worker: dual-role squads from /gac/squads/
...
- First attempt (reverted): added a per-lead scrape of `/gac/who-to-attack/` which renders individual attack-priority characters in `<div class="panel panel--size-sm">` blocks with Seen + Win %. That produced offense entries, but as 1-member "squads" (each entry is a single lead), which doesn't match how the rest of the UI presents the data.
- Final fix: the `/gac/squads/` page already contains the real top-squads data for both attack and defense — each row's **Hold %** is the defensive win rate; its complement (`1 - holdPct`) is the offensive win rate against that same squad. Rewrote the worker to scrape `/gac/squads/` only, emit one squad per table row with **both** `offenseWinRate` and `defenseWinRate` populated, and set `role: 'either'`. `recommendSquads` now has the same squads available in both buckets, ranked by the appropriate metric for each toggle.
- Season selection: walks the lookback window and filters by season parity (odd = 3v3, even = 5v5) so the first page fetched is the most recent season matching the requested bracket. In practice that's season 77 for 3v3 (current) and season 76 for 5v5.
- Verified live: `GET /?gac=3v3` returns 42 squads sourced from `/gac/squads/` (e.g. `JABBATHEHUTT lead`: 73% offense, 27% defense, 143k sample, 40.9 banners). `GET /?gac=5v5` returns 37 squads from season 76 (e.g. `LORDVADER lead`: 67% / 33%, 88k sample). Every squad has full 3 or 5 members and both win-rate fields.
- Cache note: client-side cache TTL is 12 h. Users with stale cached payloads need to tap the Refresh button on the GAC screen to pull the new data; auto-expiry will do the same thing on its own after 12 h.

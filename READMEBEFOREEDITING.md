...
- **Cold-start re-derivation.** `App.js` warm-up now calls `iap.reconcileWithPlay({ timeoutMs: 3000 })` after `premiumState.hydrate()`. Re-derives Premium directly from Google Play's `getAvailablePurchases` and overwrites the AsyncStorage cache to match. If a user flipped `@modforge/isPremium` manually (rooted device, sideloaded patched APK, emulator) or Google revoked the purchase (refund/chargeback), the cached value is corrected on next launch. IAP unavailable / Play offline / query times out → leaves cache alone so offline users on flaky connections aren't punished.
- **Native RSA-SHA1 signature verifier.** New `android/app/src/main/java/com/hungrr13/modhelper/iap/IapVerifierModule.kt` + `IapVerifierPackage.kt` (registered in `MainApplication.kt`). Module name `ModForgeIapVerifier`. Uses `Signature.getInstance("SHA1withRSA")` + `X509EncodedKeySpec` against the Play License public key; verifies against the raw `INAPP_PURCHASE_DATA` UTF-8 bytes, returns `false` on malformed Base64. JS wrapper at `src/services/iapVerifier.js` handles `dataAndroid`/`originalJson` + `signatureAndroid`/`signature` field aliases. `iap.purchaseUpdatedListener` and the new `findVerifiedPremium` helper call `verifyPurchase` before `setPremium(true)`; invalid signatures drop the event without `finishTransaction` so Play retries it.
- **Play License key slot.** `src/config/playLicense.js` exports `PLAY_LICENSE_PUBLIC_KEY` (empty by default). `iapVerifier.isConfigured()` returns false on empty — purchase listener and `reconcileWithPlay` then **fail open**: every `PREMIUM_SKU` event is trusted. Acceptable for dev/sideload. **Before producing the Play release build**, paste the Base64-encoded RSA key from Play Console → your app → Monetize → Monetization setup (older console: Setup → Licensing). Comments in the file walk through it.
- **R8 + ProGuard.** `gradle.properties`: `android.enableMinifyInReleaseBuilds=true` + `android.enableShrinkResourcesInReleaseBuilds=true`. `proguard-rules.pro`: `-keep` rules added for `com.hungrr13.modhelper.overlay.**`, `com.hungrr13.modhelper.iap.**`, the `@ReactMethod` surface on `ReactContextBaseJavaModule` subclasses, plus `java.security.**` and `javax.crypto.**` so R8 doesn't strip the verifier's reflection targets. `-dontwarn android.media.LoudnessCodecController` + `$OnLoudnessCodecUpdateListener` silences API-35+ class warnings the AdMob SDK throws against our `compileSdkVersion=34`. JS bundle is separately covered by Hermes bytecode.
- **`react-native-iap@12.16.4` wired and live.** Installed alongside the BILLING permission (`android/app/src/main/AndroidManifest.xml`) and a `missingDimensionStrategy 'store', 'play'` in `android/app/build.gradle` (the lib ships dual `play`/`amazon` flavors). RN 0.81 converted `ReactContextBaseJavaModule` to Kotlin and broke the property-style `currentActivity` accessor; both `react-native-iap` and `react-native-google-mobile-ads` needed `currentActivity → getCurrentActivity()` source patches (~7 sites). Patches in `patches/` apply automatically via `patch-package`'s `postinstall` hook. Verified on R5CX10W4LJY 2026-04-25: cold-start logcat shows three `RNIapModule: responseCode: 0` lines within 3s of bundle load (`initConnection`, `getProducts`, `getAvailablePurchases` = `reconcileWithPlay`). No FATAL EXCEPTION, no stripped-class errors, no unhandled rejections.
- **Gradle daemon JVM bumped.** `gradle.properties`: `-Xmx4g -XX:MaxMetaspaceSize=1024m` (up from 2g/512m). Adding `react-native-iap` to the classpath pushed Kotlin-compile + R8 metadata over the 512m metaspace ceiling, sending the daemon into a silent GC death-spiral (full CPU, no APK output, only visible in `~/.gradle/daemon/8.14.3/daemon-<pid>.out.log`).
- **Threat model.** Client-side gating remains fundamentally bypassable on a rooted device (user can still patch the JS bundle or call `premiumState.setPremium(true)` directly). The signature verifier raises the bar against Frida/Xposed hooks that intercept `react-native-iap` and emit synthetic purchase events; cold-start reconcile catches manual AsyncStorage flips and refunds. R8 + Hermes obscure the entry points.

### Theme persistence: hydration gate + remount-safe live cache
- `src/theme/appTheme.js` exposes a top-level `hydrateTheme()` that resolves once `@modforge/themeIsDark` has been read from AsyncStorage. Module-level `cachedIsDark` mirrors the persisted value; `toggleTheme` writes it synchronously before the AsyncStorage round-trip, so subsequent reads see the user's intent immediately.
- `App.js` warm-up `await`s `hydrateTheme()` before dropping `LoadingScreen`. A returning light-mode user no longer sees a one-frame flash of dark before the swap — the persisted value is already in `cachedIsDark` by the time `AppThemeProvider` mounts. Cold-start default for fresh installs stays dark (`useState(true)` initializer runs before hydration completes).
- `AppThemeProvider`'s mount effect resyncs to **the live `cachedIsDark` module variable**, not the value the hydrate promise originally resolved to. Android occasionally remounts the React tree inside the same JS process (Activity recreation from config changes / overlay scanner attaching), which fires the provider's `useEffect` a second time. The earlier `.then(value => setIsDark(value))` was clobbering any toggle the user made afterwards because `value` was the launch-time AsyncStorage read. Switching to `.then(() => setIsDark(cachedIsDark))` makes remounts re-sync to the current state. Verified on device: toggle to light → swipe-from-recents → reopen → still light. Commits `5e90623`, `248b270`, `44d5451`.
...
- **Populate Play License public key before Play release:** `src/config/playLicense.js` ships with `PLAY_LICENSE_PUBLIC_KEY = ''`. Empty key = signature verification is skipped (fail-open). Acceptable for dev/sideload, **not for prod**. Currently blocked on Play Console identity verification — once that completes, the field appears under Monetize → Monetization setup → Licensing. Copy the Base64 key and paste it. Consider a Gradle release-build assertion that fails if the constant is empty.

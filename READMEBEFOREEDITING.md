...
- **R8 + ProGuard.** `gradle.properties`: `android.enableMinifyInReleaseBuilds=true` + `android.enableShrinkResourcesInReleaseBuilds=true`. `proguard-rules.pro`: `-keep` rules added for `com.hungrr13.modhelper.overlay.**`, `com.hungrr13.modhelper.iap.**`, the `@ReactMethod` surface on `ReactContextBaseJavaModule` subclasses, plus `java.security.**` and `javax.crypto.**` so R8 doesn't strip the verifier's reflection targets. JS bundle is separately covered by Hermes bytecode.
- **Known build blocker.** R8 currently fails on `Missing class android.media.LoudnessCodecController (referenced from: com.google.android.gms.internal.ads.zzrz.zzb and 4 other contexts)`. The class is API 35+; current `compileSdkVersion=34`. Add `-dontwarn android.media.LoudnessCodecController` (plus any other missing classes the build reports) to `proguard-rules.pro`, or bump `compileSdkVersion` to 35.
...
- **Populate Play License public key before Play release:** `src/config/playLicense.js` ships with `PLAY_LICENSE_PUBLIC_KEY = ''`. Empty key = signature verification is skipped (fail-open). Acceptable for dev/sideload, **not for prod**. Copy the Base64 key from Play Console → Monetize → Monetization setup and paste it. Consider a Gradle release-build assertion that fails if the constant is empty.
- **Unblock R8 release build:** `:app:minifyReleaseWithR8` is failing on missing `android.media.LoudnessCodecController` (and likely siblings) referenced by the Google Mobile Ads SDK. Add `-dontwarn android.media.LoudnessCodecController` (plus any others the next build reports) to `proguard-rules.pro`, or raise `android.compileSdkVersion` from 34 to 35 in `gradle.properties`. After this clears, verify the premium hardening end-to-end on R5CX10W4LJY.

...
### GAC worker: offense recommendations via /gac/who-to-attack/
- Symptom: GAC Meta tab was empty on the Offense toggle for both 3v3 and 5v5. Client was receiving squads, but every entry had `offenseWinRate: null` so `recommendSquads().offense` returned an empty bucket.
- Root cause: the worker's offense scrape pointed at `/gac/who-to-attack/` but reused `parseGacSquadsHtml`, which expects a `<table class="stat-table">` of 3- or 5-member squad rows. The who-to-attack page has no such table — it renders a list of 50 individual lead characters in `<div class="panel panel--size-sm">` blocks, each with a `data-unit-def-tooltip-app` portrait, a "Seen" compact-number, and a "Win %" value. The old parser found zero rows and returned `[]`.
- Fix in `tools/roster-worker/worker.js`: added `parseGacWhoToAttackHtml(html)` that splits the document at each size-sm panel, extracts the unit base ID from the portrait's `data-unit-def-tooltip-app`, and reads the trailing `Seen` / `Win %` font-bold numbers. Added `scrapeGacOffense(bracket)` that walks the season window, filters by season parity (odd = 3v3, even = 5v5; current season auto-detected from embedded season IDs in the page HTML), and emits one 1-member offense "squad" per lead: `{ name: "Attack <UNIT> lead", members: [UNIT], role: 'offense', offenseWinRate, sampleSize }`. `scrapeGacSquads` now calls `scrapeGacOffense` instead of the stat-table parser for the offense role.
- Shape note: offense entries are 1-member (lead-level attack priority) while defense entries are full 3- or 5-character squads — this mirrors swgoh.gg's own page semantics. `gacMetaService.normalizeGacData` + `recommendSquads` handle 1-member squads correctly (coverage = 1/1 = 100% when the user owns the lead), so no client-side changes were needed.
- Verified live: `GET /?gac=3v3` now returns 92 squads (42 defense + 50 offense, e.g. `Attack SUPREMELEADERKYLOREN lead` at 81% win rate / 179k sample). `GET /?gac=5v5` returns 87 squads (37 defense + 50 offense) from season 76. Worker deployed via `wrangler deploy` from `tools/roster-worker/`.
- Cache note: client-side cache TTL is 12 h. Users with stale (offense-empty) cached payloads need to tap the Refresh button on the GAC screen to pull the new data; auto-expiry will do the same thing on its own after 12 h.

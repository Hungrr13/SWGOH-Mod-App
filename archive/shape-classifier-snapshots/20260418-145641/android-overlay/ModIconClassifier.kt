package com.hungrr13.modsswgoh.overlay
...
class ModIconClassifier(private val context: Context) {
  ...
  fun classify(cardBitmap: Bitmap, preferredSetProfile: String? = null): Detection {
    ...
    val setProfilesToEvaluate = when {
      preferredSetProfile != null && shapeDrivenProfiles.contains(preferredSetProfile) ->
        listOf(preferredSetProfile)
      shapeDrivenProfiles.isNotEmpty() -> shapeDrivenProfiles
      preferredSetProfile != null -> listOf(preferredSetProfile)
      else -> emptyList()
    }
  ...
  private fun detectShapeSyntheticContour(iconBitmap: Bitmap): ShapeDetectionResult? {
    ...
    try {
      ...
      val observedEvaluation =
        if (bestGuided != null && bestGuidedScore >= guidedMinScore) {
          bestGuided
        } else {
  ...
  private fun evaluateSyntheticShape(
    ...
    val geometryScores = scoreSimpleShapeRules(descriptor)
    val contourScores =
      if (contour != null) {
  ...
  private fun buildInnerFrameContour(sourceMat: Mat, grayMat: Mat): MatOfPoint? {
    ...
    return try {
      ...
      // Remove the portrait overlap and the right-side tab before tracing the inner cavity.
      Imgproc.ellipse(
        darkMat,
        Point(width * 0.16, height * 0.93),
        Size(width * 0.20, height * 0.18),
        0.0,
        0.0,
        360.0,
        Scalar(0.0),
  ...
  private fun simplifyContour(contour: MatOfPoint, epsilonRatio: Double): MatOfPoint? {
    ...
    return try {
      ...
      var workingMask = cleanupObservedShapeMask(booleanArrayFromMat(colorZoneMat, width, height), width, height)
      if (!workingMask.any { it }) return null

      guidingContour?.let { contour ->
        val contourMask = cleanupObservedShapeMask(rasterizeContourMask(contour, width, height), width, height)
      ...
      workingMask = cleanupObservedShapeMask(workingMask, width, height)
      ...
      val stabilizedMask =
        stabilizedContour?.let { cleanupObservedShapeMask(rasterizeContourMask(it, width, height), width, height) }
          ?: workingMask
  ...
  private fun buildContourDrivenSilhouetteMask(
    ...
    private fun buildInnerCavitySilhouetteMask(
      ...
      return try {
        ...
        Imgproc.ellipse(
          darkMat,
          Point(widthD * 0.16, heightD * 0.93),
          Size(widthD * 0.20, heightD * 0.18),
          0.0,
          0.0,
          360.0,
          Scalar(0.0),
        ...
        var cavityMask = cleanupInnerObservedMask(booleanArrayFromMat(darkMat, width, height), width, height)
        ...
        guidingContour?.let { contour ->
          val contourMask = cleanupInnerObservedMask(rasterizeContourMask(contour, width, height), width, height)
        ...
        cavityMask = cleanupInnerObservedMask(cavityMask, width, height)
        cavityMask =
        ...
        val stabilizedMask = cleanupInnerObservedMask(rasterizeContourMask(cavityContour, width, height), width, height)
        val finalMask = fillInternalHoles(closeRawMask(stabilizedMask, width, height, radius = 1), width, height)
  ...
  private fun cleanupObservedShapeMask(
    ...
    val portraitCenterX = width * 0.14
    val portraitCenterY = height * 0.90
    val portraitRadiusX = ((width * 0.20) * portraitScaleX).coerceAtLeast(5.0)
    val portraitRadiusY = ((height * 0.18) * portraitScaleY).coerceAtLeast(5.0)
  ...
  private fun cleanupInnerObservedMask(mask: BooleanArray, width: Int, height: Int): BooleanArray {
    val cleaned = cleanupObservedShapeMask(mask, width, height)
  ...
  private fun scoreSimpleShapeRules(descriptor: SimpleShapeDescriptor): Map<String, Double> {
    ...
    val circleScore =
      if (
        ...
        (
          near(circleCircularity, 0.78, 0.18) * 0.48 +
            near(descriptor.aspectRatio, 1.0, 0.24) * 0.16 +
            near(descriptor.extent, 0.78, 0.18) * 0.12 +
            low(geometry.centerBarStrength, 0.24, 0.72) * 0.12 +
            low(geometry.orthogonalDominance, 0.30, 0.82) * 0.12
          ).coerceIn(0.0, 1.0)
      }
    ...
    val arrowScore =
      if (
        ...
        (
          near(descriptor.vertices.toDouble(), 7.0, 3.0) * 0.24 +
            low(descriptor.circularity, 0.24, 0.62) * 0.18 +
            near(descriptor.aspectRatio, 1.30, 0.35) * 0.18 +
            near(descriptor.extent, 0.60, 0.18) * 0.14 +
            near(geometry.asymmetry, 0.22, 0.22) * 0.26
          ).coerceIn(0.0, 1.0)
      }
    ...
    profilesToEvaluate.forEach { profile ->
      cropSetSymbolVariants(iconBitmap, profile).forEach { symbolBitmap ->
        ...
        activeSetTemplates.forEach { (setName, template) ->
          ...
          val effectiveModelScore = if (isArrowBurstSet) null else modelScore
          val score = if (effectiveModelScore != null) {
            if (profile == "arrow") {
              (effectiveModelScore * 0.90) + (fallbackScore * 0.46) + burstTieBreak + burstPatchScore
            } else {
  ...
  private fun cropSetSymbolVariants(iconBitmap: Bitmap, profile: String): List<Bitmap> {
    val variants = when (profile) {
      "arrow" -> listOf(
        floatArrayOf(0.14f, 0.16f, 0.50f, 0.50f),
        floatArrayOf(0.12f, 0.14f, 0.52f, 0.52f),
        floatArrayOf(0.16f, 0.18f, 0.48f, 0.48f),
        floatArrayOf(0.10f, 0.12f, 0.54f, 0.54f),
      )
      "triangle" -> listOf(
